#!/usr/bin/env python3
"""
Direction-only Triplet Fingerprinting experiment.

This script mirrors dl_pipeline/triplet_fingerprinting.py, but it projects the
preprocessed packet tensor down to the single direction trace before training:

    source tensor: [N, 3000, F]
    model input:   [N, 3000, 1]

For datasets produced by dl_pipeline/preprocessing.py, the direction feature is
loaded from the "direction" feature name, falling back to column 3.
"""
...
class TripletFingerprintNet(nn.Module):
    """
    Paper-inspired sequence encoder for +/-1 packet direction traces.

    The input is [N, T, 1], where T is typically 3000 packets and the single
    channel contains direction values from preprocessing.py.
    """
...
def find_direction_feature_index(feature_names: Sequence[str], feature_dim: int) -> int:
    if feature_names:
        ...
        if feature_dim == 1:
            return 0
        raise ValueError(
            "Could not find a 'direction' feature in feature_names: "
    ...
    if feature_dim > 3:
        return 3
    raise ValueError(
        "Dataset has no feature_names and no obvious direction column. "
        f"Expected at least 4 features to use fallback column 3, got {feature_dim}."
    )


def to_direction_only_tensor(
    X: torch.Tensor,
    mask: torch.Tensor,
    feature_names: Sequence[str],
) -> Tuple[torch.Tensor, List[str], int]:
    if X.ndim == 2:
        if mask.ndim == 2 and X.shape != mask.shape:
            raise ValueError(
                "2D direction tensor must match mask shape: "
                f"got X={tuple(X.shape)} mask={tuple(mask.shape)}"
            )
        return X.unsqueeze(-1), ["direction"], 0

    if X.ndim != 3:
        raise ValueError(f"Expected X to have shape [N, T, F], got {tuple(X.shape)}")

    if mask.ndim == 2 and X.shape[1] == 1 and X.shape[2] == mask.shape[1]:
        return X.transpose(1, 2), ["direction"], 0

    direction_idx = find_direction_feature_index(feature_names, X.shape[-1])
    return X[..., direction_idx:direction_idx + 1], ["direction"], direction_idx
...
def load_dataset(dataset_path: Path, metadata_path: Optional[Path]) -> SequenceDataset:
    ...
    X, direction_feature_names, direction_idx = to_direction_only_tensor(
        X=X,
        mask=mask,
        feature_names=feature_names,
    )
    print(
        "Loaded direction-only input "
        f"from feature index {direction_idx}; X shape is now {tuple(X.shape)}"
    )
...
def main():
    ...
    parser.add_argument(
        "--output_dir",
        type=Path,
        default=Path("tf_experiments/direction_only_runs"),
    )
    ...
    run_summary = {
        ...
        "normalization": None,
        "input_representation": "direction_only",
        "input_shape": {
    ...
    checkpoint_path = args.output_dir / "tf_direction_only_model.pt"
